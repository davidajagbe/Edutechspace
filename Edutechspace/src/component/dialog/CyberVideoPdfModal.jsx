import React, { useState, useEffect } from 'react';
import { Dialog, DialogPanel, DialogTitle } from '@headlessui/react';
const videoresource = "/videos/jsxvideo.mp4";
// import pdfresource from "../assets/pdf/Exiat Letter.pdf";

const videoData = [
  {
    id: 1,
    title: "Cybersecurity Full Course – freeCodeCamp",
    src: videoresource,
    requirement: "No prior knowledge required – just curiosity and a willingness to learn.",
    description: "This video provides an overview of cybersecurity, its importance, the rising demand for professionals, and career paths in the field."
  },
  {
    id: 2,
    title: "Cyber Threats & Attack Types",
    src: videoresource,
    requirement: "Watch 'Introduction to Cybersecurity' first.",
    description: "Learn about different types of cyber threats like malware, phishing, DDoS attacks, and social engineering with real-world examples."
  },
  {
    id: 3,
    title: "Network Security Fundamentals",
    src: videoresource,
    requirement: "Basic understanding of computer networks is helpful but not mandatory.",
    description: "Explore how networks are secured using firewalls, VPNs, intrusion detection systems (IDS), and other protective measures."
  },
  {
    id: 4,
    title: "System & Endpoint Security",
    src: videoresource,
    requirement: "Familiarity with operating systems (Windows/Linux) recommended.",
    description: "Understand how to secure individual devices using antivirus, device hardening, software patching, and user access controls."
  },
  {
    id: 5,
    title: "Web Application Security Basics",
    src: videoresource,
    requirement: "Basic HTML/CSS/JavaScript knowledge recommended.",
    description: "Learn the common vulnerabilities in web apps like XSS, SQL Injection, and CSRF, and how to prevent them as a developer or security analyst."
  },
  {
    id: 6,
    title: "Introduction to Cryptography",
    src: videoresource,
    requirement: "No prior experience with cryptography required.",
    description: "Dive into encryption, hashing, and digital signatures. Understand how cryptographic methods secure data in transit and at rest."
  },
  {
    id: 7,
    title: "Cybersecurity Tools & Labs",
    src: videoresource,
    requirement: "Completion of earlier videos recommended for context.",
    description: "Hands-on demo of tools like Wireshark, Nmap, and Metasploit. Learn how these are used for scanning, testing, and monitoring systems."
  },
  {
    id: 8,
    title: "Cybersecurity Careers & Certifications",
    src: videoresource,
    requirement: "At least 5 prior videos should be completed to understand this better.",
    description: "Explore different career paths in cybersecurity, key certifications (CompTIA Security+, CEH, CISSP), and tips for getting started."
  }
];

const pdfData = [
  {
    id: 1,
    title: "Cybersecurity Essentials PDF",
    src: "https://www.researchgate.net/publication/333844149_Cyber_Security_Essentials"
  },
  {
    id: 2,
    title: "NIST Cybersecurity Framework (PDF)",
    src: "https://nvlpubs.nist.gov/nistpubs/SpecialPublications/NIST.SP.800-53r5.pdf"
  }
];

const CyberVideoPdfModal = ({ type, onClose }) => {
  const [currentIndex, setCurrentIndex] = useState(0);
  const isVideo = type === 'video';
  const data = isVideo ? videoData : pdfData;

  useEffect(() => {
    const handleKeyDown = (e) => {
    if (e.key === 'ArrowRight') next();
    if (e.key === 'ArrowLeft') prev();
    if (e.key === 'Escape') onClose();
    };

    window.addEventListener('keydown', handleKeyDown);
    return () => window.removeEventListener('keydown', handleKeyDown);
  }, [currentIndex]);

  useEffect(() => {
    const savedIndex = localStorage.getItem(`current-${type}-index`);
    if (savedIndex) setCurrentIndex(parseInt(savedIndex));
  }, [type]);

  useEffect(() => {
    localStorage.setItem(`current-${type}-index`, currentIndex);
  }, [currentIndex, type]);

  const next = () => {
    if (currentIndex < data.length - 1) setCurrentIndex(currentIndex + 1);
  };
  const prev = () => {
    if (currentIndex > 0) setCurrentIndex(currentIndex - 1);
  };

  return (
    <Dialog open={true} onClose={onClose} className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-40 z-50 p-4">
      <DialogPanel className="bg-white p-6 rounded-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto shadow-xl">
        <DialogTitle className="text-xl sm:text-2xl font-bold mb-4 text-center">{data[currentIndex].title}</DialogTitle>
        <div className="flex justify-between items-center flex-wrap gap-4">
          <button onClick={prev} disabled={currentIndex === 0} className="px-4 py-2 rounded bg-gray-200 disabled:opacity-50">Previous</button>
          <span className="text-sm sm:text-base">{currentIndex + 1} / {data.length}</span>
          <button onClick={next} disabled={currentIndex === data.length - 1} className="px-4 py-2 rounded bg-gray-200 disabled:opacity-50">Next</button>
        </div>

        {isVideo ? (
          <video controls className="w-full h-[200px] sm:h-[350px] md:h-[450px] lg:h-[500px] rounded mb-4" src={data[currentIndex].src}></video>
        ) : (
          <iframe
            title={data[currentIndex].title}
            src={data[currentIndex].src}
            className="w-full h-[400px] sm:h-[500px] rounded mb-4"
          ></iframe>
        )}
        
        {/* Progress Indicator */}
        <div className="flex justify-center gap-2 my-4">
          {data.map((_, idx) => (
            <span
              onClick={() => setCurrentIndex(idx)}
              key={idx}
              className={`h-2 w-2 rounded-full cursor-pointer transition-all duration-300 ${idx === currentIndex ? 'bg-blue-900 scale-125' : 'bg-gray-300'}`}
            />
          ))}
        </div>

        {/* Course Outline */}
        <div className="bg-gray-100 p-4 rounded-md mb-4">
            <h3 className="text-lg font-semibold mb-2">Requirement</h3>
            <p className="text-sm text-gray-700 mb-3">{data[currentIndex].requirement}</p>

            <h3 className="text-lg font-semibold mb-2">Description</h3>
            <p className="text-sm text-gray-700">{data[currentIndex].description}</p>
        </div>

        <button onClick={onClose} className="mt-6 w-full bg-blue-950 text-white py-3 rounded-lg text-lg hover:bg-blue-800 transition">Close</button>
      </DialogPanel>
    </Dialog>
  );
};

export default CyberVideoPdfModal;
